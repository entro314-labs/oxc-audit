...
tester.run(
	"slop-stop/require-safety-comment-for-type-assertion",
	requireSafetyCommentForTypeAssertionRule,
	{
		...
		invalid: [
		{
			// One chain, one justification: the inner link must not raise a second diagnostic
			// at the same source position.
			name: "an assertion chain reports once",
